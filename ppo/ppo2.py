...
class Agent():
    ...
    def learn(self, mini_batch_size, states_batch, actions_batch, log_probs_batch, returns_batch, advantages_batch, clip_param=0.2):
        num_states = states_batch.size(0)
        num_mini_batches = num_states // mini_batch_size
        for _ in range(self.epochs_per_train):
            for _ in range(num_mini_batches):
                mini_batch_indices = np.random.randint(0, num_states, mini_batch_size)
                state     = states_batch[mini_batch_indices, :]
                action    = actions_batch[mini_batch_indices, :]
                old_log_probs  = log_probs_batch[mini_batch_indices, :]
                return_    = returns_batch[mini_batch_indices, :]
                advantage = advantages_batch[mini_batch_indices, :]
