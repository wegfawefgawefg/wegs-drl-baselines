...
def play_test_episode(agent, env, stats):
    with torch.no_grad():
        ...
        while not done:
            ...
            state = state.view(1, 1, agent.lstm.data_shape)
            output, (hidden_state, cell_state) = agent.lstm(state, (hidden_state, cell_state))
            state = output[:, -1, :]
...
if __name__ == "__main__":
    ...
    rollout_collector = RolloutCollector(
        num_env_workers=8, make_env_func=make_env, agent=agent, batch_size=32, rollout_length=16, num_recurrence_steps=4,
