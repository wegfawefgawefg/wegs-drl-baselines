...
class Agent:
    ...
    def learn(self):
        ...
        with torch.no_grad():
            qs_ = self.net_(states_)        
            actions_ = qs_.argmax(dim=1)    
            dists_ = self.net_.dist(states_) #
            action_dist_ = dists_[batch_index, actions_] 
            ...
            proj_dist.view(-1).index_add_(  #[64,8]
                0, 
                (l + offset).view(-1), 
                (action_dist_ * frac).view(-1)
            )
            proj_dist.view(-1).index_add_(  #[64,8]
                0, 
                (u + offset).view(-1), 
                (action_dist_ * dec_frac).view(-1)
            )
