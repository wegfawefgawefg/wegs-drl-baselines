...
class Agent():
    ...
    def learn_at_end_of_slice(self, stats):
        if self.slice_replay_buffer.count < self.batch_size:
            return 

        self.net.train()

        states, actions, rewards, states_, dones = self.slice_replay_buffer.sample(self.batch_size, self.device)
        ...
        chosen_q = qs[batch_indices, last_actions.T[0]]
        ...
        rewards = rewards.view(self.batch_size, self.slice_size)
    ...
    def learn_with_slice_mixing(self, stats):
        if self.slice_replay_buffer.count < self.batch_size:
            return 
    ...
    def learn(self, stats):
        self.learn_at_end_of_slice(stats)